...
s1 = SourceLLMWDC( -17.84, 28.56,  10.0,  0.1,    4,   3.9, 243.3627, 31.597,  -73.886 )
...
cfg = OpenSWPCConfig(vmod,
                        ...
                        # save every grid point, and every 10 timesteps:
                        ntdec_s=10,idec=1, jdec=1, kdec=1,  

                        # waveform output
                        sw_wav_u=true,sw_wav_v=false, ntdec_w=10,
...
dat,t=read_volume(joinpath(cfg.odir, "swpc.3d.vol.v.nc"), timestep=ntsteps_saved)
